...
structure PrimopId : PRIMOP_ID =
  struct
    ...
    fun isPrimCallcc (Prim p) = (case PrimopBind.defnOf p
           of P.PRIM CP.CALLCC => true
            | P.PRIM CP.CAPTURE => true
            |  _ => false
          (* end case *))
    ...
    fun selStrPrimId ([], slot) = []  (* not a bug? DBM *)
      | selStrPrimId (elems, slot) = ((case List.nth(elems, slot)
           of StrE elems' => elems'
            | PrimE _ => bug "PrimopId.selStrPrimId: unexpected PrimE"
          (* end case *))
            (* This bug happens if we got a primid for a value component when
             * we expected a str_prim_elem for a structure
             *)
    ...
    fun selValPrimFromStrPrim ([], slot) = NonPrim (* not a bug? DBM *)
      | selValPrimFromStrPrim (elems, slot) = ((case List.nth(elems, slot)
           of PrimE(id) => id
            | StrE _ => bug "PrimopId.selValPrimFromStrPrim: unexpected StrE")
          (* end case *))
            (* This bug occurs if we got a substructure's str_prim_elem instead
             * of an expected value component's prim_id
             *)
