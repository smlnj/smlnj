...
structure InlineOps =
  struct
    ...
    datatype t
      ...
      | LSHIFT of int    		(* left shift *)
      | RSHIFT of int    		(* right shift *)
      | RSHIFTL of int    		(* right shift logical *)
      | CNTZ of int                     (* count zeros *)
      | CNTO of int                     (* count ones (aka population count) *)
      | CNTLZ of int                    (* count leading zeros *)
      | CNTLO of int                    (* count leading ones *)
      | CNTTZ of int                    (* count trailing zeros *)
      | CNTTO of int                    (* count trailing ones *)
      | CEIL_LOG2 of int                (* log2 rounded up *)
 ...
 *)
    ...
    local
      ...
      fun withSize (p, sz) = concat[p, "_u", Int.toString sz]
    in
    fun toString p = (case p
           of DIV nk => withKind ("div", nk)
            ...
            | LSHIFT sz => withSize ("shiftLeft", sz)
            | RSHIFT sz => withSize ("shiftRight", sz)
            | RSHIFTL sz => withSize ("lShiftRight", sz)
            | CNTZ sz => withSize ("cntZeros", sz)
            | CNTO sz => withSize ("cntOnes", sz)
            | CNTLZ sz => withSize ("cntLeadingZeros", sz)
            | CNTLO sz => withSize ("cntLeadingOnes", sz)
            | CNTTZ sz => withSize ("cntTrailingZeros", sz)
            | CNTTO sz => withSize ("cntTrailingOnes", sz)
            | CEIL_LOG2 sz => withSize ("ceilLog2", sz)
