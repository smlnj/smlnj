...
structure UnpickMod : UNPICKMOD = struct
    ...
    fun mkSharedStuff (session, lvar) = let
        ...
        fun inlineop () = let
            fun inl #"\000" = InlP.DIV(numkind())
              ...
              | inl #"\004" = InlP.LSHIFT(int())
              | inl #"\005" = InlP.RSHIFT(int())
              | inl #"\006" = InlP.RSHIFTL(int())
              | inl #"\007" = InlP.CNTZ(int())
              | inl #"\008" = InlP.CNTO(int())
              | inl #"\009" = InlP.CNTLZ(int())
              | inl #"\010" = InlP.CNTLO(int())
              | inl #"\011" = InlP.CNTTZ(int())
              | inl #"\012" = InlP.CNTTO(int())
              | inl #"\013" = InlP.CEIL_LOG2(int())
              | inl #"\014" = InlP.MIN(numkind())
              | inl #"\015" = InlP.MAX(numkind())
              | inl #"\016" = InlP.ABS(numkind())
              | inl #"\017" = InlP.CHR
              | inl #"\018" = InlP.MKARRAY
              | inl #"\019" = InlP.SUBSCRIPT
              | inl #"\020" = InlP.SUBSCRIPTV
              | inl #"\021" = InlP.UPDATE
              | inl #"\022" = InlP.UNBOXEDUPDATE
              | inl #"\023" = InlP.NUMSUBSCRIPT(numkind())
              | inl #"\024" = InlP.NUMSUBSCRIPTV(numkind())
              | inl #"\025" = InlP.NUMUPDATE(numkind())
              | inl #"\026" = InlP.NOT
              | inl #"\027" = InlP.COMPOSE
              | inl #"\028" = InlP.BEFORE
              | inl #"\029" = InlP.IGNORE
              | inl #"\030" = InlP.IDENTITY
              | inl #"\031" = InlP.HOST_WORD_SIZE
              | inl #"\032" = InlP.HOST_BIG_ENDIAN
              | inl _ = raise Format
            in
              share inlM inl
            end
        ...
        fun commonop () = let
            ...
            fun cop #"\000" = CP.FSGN(int())
              ...
              | cop #"\024" = CP.BOXED
              | cop #"\025" = CP.UNBOXED
              | cop #"\026" = CP.IS_POW2(int())
              | cop #"\027" = CP.CAST
              | cop #"\028" = CP.REAL_TO_BITS(int())
              | cop #"\029" = CP.BITS_TO_REAL(int())
              | cop #"\030" = CP.GETHDLR
              | cop #"\031" = CP.SETHDLR
              | cop #"\032" = CP.GETVAR
              | cop #"\033" = CP.SETVAR
              ...
              | cop #"\039" = CP.DEREF
              | cop #"\040" = CP.ASSIGN
              | cop #"\041" = CP.UNBOXEDASSIGN
              | cop #"\042" = CP.OBJLENGTH
              | cop #"\043" = CP.GETTAG
              | cop #"\044" = CP.MKSPECIAL
              | cop #"\045" = CP.SETSPECIAL
              | cop #"\046" = CP.GETSPECIAL
              | cop #"\047" = CP.NEW_ARRAY0
              | cop #"\048" = CP.GET_SEQ_DATA
              | cop #"\049" = CP.SUBSCRIPT_REC
              | cop #"\050" = CP.SUBSCRIPT_RAW64
              | cop #"\051" = CP.CPTR_TO_WORD
              | cop #"\052" = CP.WORD_TO_CPTR
              | cop #"\053" = CP.RAW_LOAD(numkind())
              | cop #"\054" = CP.RAW_STORE(numkind())
              | cop #"\055" = CP.RAW_CCALL NONE
              | cop #"\056" = CP.RAW_CCALL(SOME(ccall_info()))
              | cop #"\057" = CP.RAW_RECORD{align=int()}
              | cop _ = raise Format
            in
              share comM cop
            end
    ...
    fun mkEnvUnpickler extraInfo sessionInfo context = let
        ...
        local
          fun look what lk (m, i) = let
              val (mapping, descr) = context m
              in
                case lk (mapping, i)
                 of SOME x => x
                  | NONE => (
                    ErrorMsg.impossible (concat [
                        "UnpickMod: stub lookup failed for ",
                         what, " in ", descr (),
                         ": m = ",
                         case m of
                             NONE => "NONE"
        ...
        and nameRepDomain () = let
            fun n #"d" = { name = symbol (), rep = conrep (), domain = tyoption () }
              | n _ = raise Format
            in
              share nrdM n
            end
        ...
        and tycon' () = let
            val tyc = tycon ()
            val tree = (case tyc of T.GENtyc r => M.TYCNODE r | _ => notree)
            in
              (tyc, tree)
            end
        ...
        and ty' () = let
            fun t #"a" = let
                ...
                in
                  (T.CONty (tyc, tyl), branch [tyctr, tyltr])
                end
              ...
              | t #"d" = let
                ...
                val (b, btr) = ty' ()
                in
                  (T.POLYty{ sign = s, tyfun = T.TYFUN{ arity = ar, body = b } }, btr)
                end
        ...
        and var' () = let
            fun v #"1" = let
              ...
              | v #"2" = let
                ...
                val (vl,vltr) = varlist' ()
                in
                  (V.OVLDvar{ name = n, variants = vl}, vltr)
                end
        ...
        and Signature' () = let
            fun sg #"A" = (M.ERRORsig, notree)
              | sg #"B" = let
                val sr = lookSig (libModSpec (), sigId ())
                in
        ...
        and spec' () = let
            fun sp #"1" = let
                val (i, itr) = tycSpecInfo' ()
                in
                  (M.TYCspec { entVar = entVar (), info = i }, itr)
                end
              | sp #"2" = let
                val (s, str) = Signature' ()
                in
              ...
              | sp #"3" = let
                val (f, ftr) = fctSig' ()
                in
                  (M.FCTspec { sign = f, slot = int (), entVar = entVar () }, ftr)
                end
              | sp #"4" = let
                val (t, ttr) = ty' ()
                in
        ...
        and tycSpecInfo' () = let
            fun tsi #"a" = let
                val (t,ttr) = tycon' ()
                in
                  (M.RegTycSpec{spec = t, repl = bool (), scope = int ()}, ttr)
                end
              | tsi #"b" = (M.InfTycSpec{name = symbol (), arity = int ()}, notree)
              | tsi _ = raise Format
            in
              share tsiM tsi
            end
        ...
        and fctClosure' () = let
            fun f #"f" = let
                ...
                val (e, etr) = entityEnv' ()
                in
                  (M.CLOSURE { param = p, body = b, env = e }, branch [btr, etr])
                end
              | f _ = raise Format
            in
              share fctcM f
        ...
        and fctExp' () = let
            fun fe #"o" = (M.VARfct (entPath ()), notree)
              ...
              | fe #"r" = let
                ...
                val (s, str) = fctSig' ()
                in
                  (M.LAMBDA_TP { param = p, body = b, sign = s }, branch [btr, str])
                end
              | fe #"s" = let
                val (d, dtr) = entityDec' ()
                val (f, ftr) = fctExp' ()
        ...
        and entityEnv' () = let
            fun eenv #"A" = let
              ...
              | eenv #"D" = let
                val r = lookEnv (libModSpec (), envId ())
                in
    ...
    fun unpickleEnv context (hash, pickle) = let
        val session = UU.mkSession (UU.stringGetter (Byte.bytesToString pickle))
