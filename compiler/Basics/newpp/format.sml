...
structure Format =  (* no signature, exports only datatypes *)
struct
 ...
 *  (1) as explicit separators (turned into "elements" by the SEP constructor) in special blocks (SBLOCK constructor);
 ...
 * "tightly" concatenate a sequence of formats so that their layouts will abut one another (see the concat and
...
datatype separator
  ...
  | SoftLine of int  (* soft line break (n >= 0); rendered as n spaces, if the line break is not triggered;
		      * if the line break is triggered because the following format does not fit in remaining line space,
                      * renders as newline + indent to current block left margin (blm) *)
 ...
 *
 *  -- BLOCK: _aligned_ blocks with separator and bindent attributes. The alignment can be horizontal (Space separator),
 *     vertical (HardLine), or packed (SoftLine).
 *
 ...
 *
 *  There is also a FLAT format constructor. This is used to specify that a format should be rendered (and measured)
 *  _flat_, without newlines (as though rendered on an infinite line with hard line breaks rendered as spaces.
 *
 *  Finally, there is a "conditional" format constructor, ALT (format1, format2). This renders as format1 if it fits,
 *  otherwise renders format2.
 *
 *    Note: See the definition of the "tryFlat" function (in NewPP) to see an example of how FLAT and ALT
 *    can be used together.
 *
 *    Note: ALT and FLAT are related to Wadler's Union documents and the FlatAlt constructor in recent Wadler-Leijen
 *    prettyprinters. In a format of the form ALT(format1, format2) it will usually be the case that the alternative
 *    formats format1 and format2 have the same content (text strings), but this is not enforced, and in some cases it
 *    may desired that format2 is an abbreviated version of format1.
 *
...
(* alignmentSeparator : alignment -> separator
 * -- only value exported from Format (internal use only, in Render, NewPP) *)
fun alignmentSeparator H = Space 1
  | alignmentSeparator V = HardLine
  | alignmentSeparator P = SoftLine 1
...
(* NOTES:
  ...
  (1) CONTEXT or CONTEXTUAL formats (not implemented). 
    ...
    unavailable when measuring the format (i.e. it is not "structural" information
