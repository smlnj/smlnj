...
struct
	...
	(case oper
	   of P.CMP{oper, signed, sz} =>
	    ...
	    | P.PNEQ => PP.text "pneq"
	    | P.LIMIT n =>
	        PP.hcat (PP.text "needGC",
			 PP.parens (PP.text (Word.fmt StringCvt.DEC n)))
	(* end case *))
    ...
    fun fmtAlloc (P.RECORD{desc, mut=false}) =
      ...
      | allocToString (P.RAW_RECORD{desc, ...}) =
    ...
    local fun fmtBr (P.LIMIT 0w0, []) = PP.text "needsGC"
       ...
       in fun fmtStm stm =
		case stm
		 of C.LET(e, x, stm) =>
		      PP.pcat (PP.hblock [fmtExp e, PP.text "->" fmtParam x],
			       PP.indent 2 (fmtStm stm))
		  | C.ALLOC(p as P.RAW_ALLOC _, [], x, stm) =>
		      PP.pcat (PP.hblock [fmtAlloc p, PP.text "->", fmtLvar x],
			       PP.indent 2 (fmtStm stm))
		  | C.ALLOC(p, args, x, stm) => 
		      PP.pcat (PP.hblock [fmtApp (fmtAlloc p, args), PP.text " -> ", fmtLvar x],
		               PP.indent 2 (fmtStm stm))
		  | C.APPLY(f, args, tys) => 
	...
	let val kindString =
	 ...
	 in PP.vblock
	      [PP.hblock 
	         [PP.text kindString, PP.ccat (PP.text "(L)", fmtLvar lab),
    ...
    fun fmtCompUnit {srcFile, entry, fns} =
	  PP.vblock
	    [PP.hcat (PP.text "##########", srcFile),
	     fmtCluster entry;
	     PP.vblock (map (fn f => (PP.vcat (PP.text "#####", fmtCluster f))) fns;
	     PP.text "##########"]
