...
*)
  ...
  (* getName : path -> S.symbol option *)
  (* get the name of a binding, checking for empty path *)
  fun getName (SPATH syms: path) =
      (case syms
	 of nil => NONE
	  | _ => SOME (List.last syms)) (* the actual last symbol in the SymPath.path *)
...
struct
  ...
  fun getName (IPATH syms: path) =
      (case syms
	of nil => NONE
	 | sym :: _ => SOME sym) (* "last", i.e. first symbol in the InvPath.path *)
