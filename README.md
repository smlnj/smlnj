...
This is the main development repository for **SML/NJ**.  Under active development, it is not considered stable.  As such, we recommend the
[legacy version](https://github.com/smlnj/legacy) of the system for production use cases.

As of the 2025.3 release, this version will work on AMD64
...
**_Note_** The build process has changed from the prior legacy system.
...
1. Switch to the [`system`](system/) directory and run the `cmb-make` command:
    ...
    command in the user's `PATH` takes precedence. When appropriate, you can also specify a different path to an `sml` command.
...
3. Install the system
    ...
    ```
    The `-boot` option is new (and optional) in this version. It will
    overwrite existing boot files in the root directory
    (*e.g.*, `boot.amd64-unix.tgz`) with files generated by the `cmb-make` 
    command.
