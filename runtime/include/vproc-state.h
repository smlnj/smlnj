...
struct vproc_state {
    ...
    Addr_t	vp_limitPtrMask;        /* for raw-C-call interface; when a C function
                                         * call returns, we set the limit pointer to
                                         * `limitPtr & vp_limitPtrMask`.  If a signal
                                         * is delivered during the C call, the handler
