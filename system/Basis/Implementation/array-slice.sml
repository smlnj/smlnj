...
structure ArraySlice : ARRAY_SLICE =
   ...
   *)
    datatype 'a slice = SL of ('a array * int * int)
  ...
  (* create a vector of the given lenght initialized to the given list of elements *)
    val create = Assembly.A.create_v
  ...
  (* FIXME: this is inefficient (going through intermediate list) *)
    fun vector (SL(base, start, len)) = let
	  fun mk (i, xs) = if (start <= i)
		then mk (i -- 1, usub(base, i) :: xs)
		else create (len, xs)
	  in
	    mk (start ++ len -- 1, [])
	  end
(***** A better implementation once we have the correct Real64Vector representation
