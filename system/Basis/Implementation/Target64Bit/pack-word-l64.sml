...
structure PackWord64Little : PACK_WORD =
  ...
  (* convert the byte length into word64 length (n div 8), and check the index *)
    ...
    (* make a 64-bit word value from a sequence of bytes in increasing memory-address
     * order assuming a little-endian layout; i.e., `b1` is LSB and `b8` is MSB.
     *)
    fun mkWord (b1, b2, b3, b4, b5, b6, b7, b8) = let
	  val w = w8ToW64 b8 (* MSB last *)
	  ...
	  val w = W64.orb(W64.lshift(w, 0w8), w8ToW64 b1) (* LSB first *)
	  in
	    w
	  end
    ...
    fun update (arr, i, w) = let
	  ...
	  val k = scale i
	  in
	    W8A.update (arr, k,    w64ToW8 w); (* LSB first *)
	    ...
	    W8A.update (arr, k++7, w64ToW8(W64.rshiftl(w, 0w56))) (* MSB last *)
	  end

  end (* structure PackWord64Little *)
end (* local *)
