...
in
   ...
   (* pickling and unpickling paths
    ...
    fun unpicklePath ("%u" :: arcs) = (ABS U, arcs)
      | unpicklePath ("%w" :: vol :: arcs) = (ABS (W vol), arcs)
   ...
   (* files and stable ids *)
    ...
    (* tstamp : file -> TSamp.t *)
    fun tstamp ({path, ...} : file) = TStamp.fmodTime (pathToFpath path)
   ...
   (* current working directory (CDW) *)
    ...
    local (* global anchor -> path environment *)
    ...
    in 
      ...
      fun setAnchor (anchor, pathOp) : unit =
	  case find anchor
	    of SOME _ =>
		 anchorMapRef :=
		   (case pathOp
	...
	setAnchor (anchor, Option.map (mkAbsolute relativeTo) pathOp)
    ...
    fun reset_anchors () = (resetAnchors (); sync ())
     ...
     *
     * Why do parseNative and parseStandard need to expand anchors of anchored paths? *)

    (* parseNative0 : fpath -> path *)
    (* parse a quoted fpath in "native" format -- we delegate to parseFpath, which calls
     * OS.Path.fromString to do the original parsing.
     * This may work ok when the native OS is Unix. What about Windows?
     * Do we need to write complete "native" file path parsers? For both Unix and Windows? *)
    val parseNative0 = parseFpath

    (* parseStandard0 : fpath -> path *)
    ...
    (* parseNative : pathEnv -> fpath -> path *)
    (* parse a file path string into a path, assuming "native" file path notation
     * "expand" an anchor head (which could result in another anchor headed path).*)
    fun parseNative pathEnv fpath =
          (case parseNative0 fpath
    ...
    (* parseStandard : pathEnv -> fpath -> path *)
    (* parse a file path string into a path, assuming "standard" file path notation,
     * "expand" an anchor head (which could result in another anchor headed path).*)
    fun parseStandard pathEnv fpath =
          (case parseStandard0 fpath
             of path as ((REL _ | ABS _), _) => path
              | (ANC anchor, arcs) => (* "expand" the anchor & extend with arcs *)
		  (case lookAnchor (pathEnv, anchor)
    ...
    (* parseSegmented : pathEnv -> fpath -> path
     ...
     * Why does parseSegmented need to expand the anchor of an initial anchored segment? *)

    (* parseSegmented [decode] : pathEnv -> fpath -> path *)
    ...
    fun parseSegmented (pathenv: pathEnv) (fpath: fpath) : path =
...
--------------------------------------------------------------------------------
13. Phase 8: reorganizing SrcPath

Preparation for possibly splitting out Path and File structures.

Slightly simplifying path pickling/unpickling, using string list rather than string list list.

Still unclear about what _really_ needs to be done about _native_ file path parsing.

Still unclear about why parseNative, parseStandard and parseSegmented file path parsing
functions need to take a pathEnv argument and use it to "expand" an anchor head.
