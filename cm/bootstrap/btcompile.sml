...
functor BootstrapCompileFn (
    ...
    fun mkBootList g = let
        fun listName p = (case P.fromString p
               of { vol = "", isAbs = false, arcs = _ :: arc1 :: arcn } => (
                    case os
                     of SMLofNJ.SysInfo.WINDOWS =>
                          concat (arc1 :: foldr (fn (a, r) => "\\" :: a :: r) [] arcn)
    ...
    val checkDirbase = let
          val prev = ref NONE
          fun ck db = (case !prev
                   of NONE => prev := SOME db
                    | SOME db' =>
                        if db = db' then ()
                        else (
                          Say.vsay ["[new dirbase is `", db, "'; CMB reset]\n"];
                          internalReset ();
                          prev := SOME db)
                  (* end case *))
          in
    ...
    fun mkCompile { master, root, dirbase = dbopt } = let
        ...
        fun mkMainCompile arg = let
            ...
            in
                GG.GROUP { exports = foldl addExports specialExports others,
                           kind = GG.LIB {
            ...
            fun tryLoadInitGroup () = (case loadInitGroup ()
                   of SOME g => g
                    ...
                    fun finish (g, gp) = let
                        ...
                        fun writePid s i = let
                            ...
                            val bfc = BFC.getStable { stable = sn, offset = os,
                                                      descr = descr }
                        in
                            case Binfile.exportPidOf bfc
                             of NONE => ()
                              | SOME pid => TextIO.output (s, concat [
                                    " ", Int.toString os, ":", PersStamps.toHex pid
                                  ])
                        end
    ...
    in
        case BuildInitDG.build ginfo initgspec of
            SOME x => mkMainCompile x
          | NONE => NONE
    end

    fun compile dbopt = (
          StabModmap.reset ();
          case mkCompile { master = true, root = NONE, dirbase = dbopt }
           of NONE => false
            | SOME (_, thunk) => thunk ()
          (* end case *))

    local
      fun slave NONE = (internalReset (); NONE)
        | slave (SOME(dirbase, root)) = (
            StabModmap.reset ();
            #set (SSV.symval "CMB_SLAVE_MODE") (SOME 1);
            #set (SSV.symval "NO_PLUGINS") (SOME 1);
            case mkCompile { master = false, root = SOME root, dirbase = SOME dirbase }
             of NONE => NONE
  ...
  end (* functor BootstrapCompileFn *)
