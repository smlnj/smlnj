...
in
    ...
    (* Given a "bindings", stick a parallel Bind in front of decls, the given decl list.
     * While doing so, if a Ref occured at the front of decls, move it to the front
	...
	(case dec
	   of ValDec (vbs, _) => foldl vb_dl decls vbs
	    ...
	    | DataReplDec (_,cn) => dl_addS (s_addMP (cn, SS.empty), decls)
		...
		let (* collector : Ast.strb * (symset * bindings) -> (symset * bindings) *)
		    fun collector (MarkStrb (arg, _), x) = collector (arg, x)
		      | collector (Strb { name, def, constraint }, (ss, binds)) =
			  let val (ss', mexp) = pairOp (strexp_p def, sigexpc_p constraint)
			   in (SS.union (ss, ss'), (name, mexp) :: binds)
			  end
		 in parbind collector strbs decls
		end
	    | FctDec fctbs =>
		let (* collector : Ast.fctb * (symset * bindings) -> (symset * bindings) *)
		    fun collector (MarkFctb (arg, _), x) = collector (arg, x)
		      | collector (Fctb { name, def }, (ss, binds)) =
			  let val (ss', mexp) = fctexp_p def
			   in (SS.union (ss, ss'), (name, mexp) :: binds)
			  end
		 in parbind collector fctbs decls
		end
	    | SigDec sigbs =>
		let (* collector : Ast.sigb * (symset * bindings) -> (symset * bindings) *)
		    fun collector (MarkSigb (arg, _), x) = collector (arg, x)
		      | collector (Sigb { name, def }, (s, bl)) =
			  let val (s', e) = sigexp_p def
			   in (SS.union (s, s'), (name, e) :: bl)
			  end
		 in parbind collector sigbs decls
		end
	    | FsigDec fsigbs =
	...
	let fun sameReg (LocalDec (_, body), k) = sameReg (body, k)
	      ...
	      | sameReg (OpenDec _, k) =
		  (fn () => (k ();
			     (* err EM.COMPLAIN region "toplevel open" *)
			     error "toplevel open"))
	      | sameReg (_, k) =
		 (fn () => (k ();
			    (* err EM.WARN region "definition not tracked by CM" *)
    ...
    fun convert (dec: Ast.dec) : SK.decl =
	seq (dec_dl (dec, nil))
