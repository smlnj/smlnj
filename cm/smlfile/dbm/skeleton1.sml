(* cm/smlfile/dbm/skeleton1.sml
 * SML source skeletons, revised.
 *
...
in
  ...
  datatype decl
    ...
    | Local of decl * decl
    | Par of decl list  (* parallel bindings -- no shadowing *)
    | DSeq of decl list (* sequential declarations -- may shadow *)
    | Open of S.symbol list   (* open a single path *)
    | FPHSd of SS.set          (* Free Path Heads: not empty *)

  and exp
    = (* Var of S.symbol list   (* symbol list (sympath) not null *)
    | *) Str of decl list     (* implicit Seq, decl list not null? *)
    | Let of decl list * exp (* implicit Seq, decl list not null? *)
    | FPHSe of SS.set          (* Free Path Heads: not empty *)
(*    | Pair of exp * exp      (* pair of exps *) *)
    | ESeq of exp list      (* list of exps -- replaces Pair *)

(* a "null" exp would be ESeq nil *)
