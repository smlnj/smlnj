...
in 
  ...
  (* reach : ? *)
  fun reach ops (export_nodes: DG.sbnode list) =
      let val { add, member, empty } = ops
	  fun snode (x as DG.SNODE n, (known, stabfringe)) =
	      let val { smlinfo = i, localimports = l, globalimports = g } = n
	       in if member (known, i)
	  ...
	  and sbnode (DG.SB_BNODE (DG.BNODE n, _, _), (known, stabfringe)) =
       ...
       in foldl sbnode (empty, StableSet.empty) export_nodes
      end
  ...
  (* snodeMap' : ? *)
  fun snodeMap' (exports: DG.impexp SymbolMap.map, acc) =
      let fun add (m, i, x) = SrcPathMap.insert (m, SmlInfo.sourcepath i, x)
  ...
  (* snodeMap : ? -> ? *)
  fun snodeMap g =
      let fun snm (GG.ERRORGROUP, x) = x
	    | snm (g as GG.GROUP grec, (a, seen)) =
	      let val { exports, sublibs, grouppath, ... } = grec
	       in if SrcPathSet.member (seen, grouppath)
		  then (a, seen)
		  else foldl (fn ((_, g, _), x) => snm (g (), x))
			 (snodeMap' (exports, a), SrcPathSet.add (seen, grouppath))
			 sublibs
	      end

       in #1 (snm (g, (SrcPathMap.empty, SrcPathSet.empty)))
      end

  (* groupsOf : ? -> ? *)
  fun groupsOf g =
      let fun subgroups (GG.GROUP { kind = GG.NOLIB x, ... }) =
		#subgroups x
	  ...
	  fun go (GG.ERRORGROUP, a) = a
	    | go (g as GG.GROUP { grouppath, ... }, a) =
		let val sgl = subgroups g
		    fun sl ((p, gth, _), a) =
			(case gth ()
			   of g as GG.GROUP { kind = GG.NOLIB _, ... } =>
			      if SrcPathSet.member (a, p) then a else go (g, a)
			    | _ => a)

		 in FileSet.add (foldl sl a sgl, grouppath)
		end

       in go (g, SrcPathSet.empty)
      end

  (* stableLibsOf : GG.group -> ? *)
  fun stableLibsOf GG.ERRORGROUP = SrcPathMap.empty
	...
	let fun slo' ((_, GG.ERRORGROUP), x) = x
		  ...
		  let val { kind, sublibs, ... } = grec
		   in if SrcPathSet.member (seen, p)
		      then (seen, res)
	 ...
	 in #2 (slo' ((grouppath, g), (SrcPathSet.empty, SrcPathMap.empty)))
	end

  (* frontier : ? -> GG.group -> ? *)
  fun frontier _ GG.ERRORGROUP = StableSet.empty
    | frontier inSet (GG.GROUP { exports, ... }) =
	let fun bnode (DG.BNODE n, (seen, f)) =
		let val i = #bininfo n
		    val li = #localimports n
		 in if StableSet.member (seen, i)
		    then (seen, f)
		    else let val seen = StableSet.add (seen, i)
			  in if inSet i
			     then (seen, StableSet.add (f, i))
		...
		(case nth ()
		   of (_, DG.SB_BNODE (n, _, _)) => n :: bnl
		    | _ => bnl)

	    val bnl = SymbolMap.foldl get_bn [] exports

	 in #2 (foldl bnode (StableSet.empty, StableSet.empty) bnl)
	end
