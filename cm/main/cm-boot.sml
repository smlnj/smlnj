...
functor LinkCM (structure HostBackend : BACKEND) =
  struct
    ...
    local
      ...
      val my_archos = concat [HostBackend.architecture, "-", FilenamePolicy.kind2name os]

      structure SSV = SpecificSymValFn (
          val arch = HostBackend.architecture
          val os = os
	...
	| make_runner add_bindings gp (g as GG.GROUP grec) = let
		...
		| SOME { stat } => (
		   ...
		   * use of.) *)
		    ...
		    case l_group gp
			  ...
			  if add_bindings
                            then let
                              val delta = E.mkenv { static = stat, dynamic = dyn }
                              val loc = ER.loc ()
                              val base = #get loc ()
      ...
      structure Stabilize = StabilizeFn (
          val arch = HostBackend.architecture
          structure StabModmap = StabModmap
          fun recomp gp g = let
                val { store, get } = BFC.new ()
      ...
      fun getPending () =
            List.map (Symbol.describe o #1) (SymbolMap.listItemsi (AutoLoad.getPending ()))
      ...
      fun initPaths () = let
		  ...
		  val work = SrcPath.processSpecFile { env = penv, specfile = f, say = Say.say }
	          in
		    SafeIO.perform {
                        openIt = fn () => TextIO.openIn f,
            ...
            in
              List.app processOne p;
              SrcPath.sync ()
            end

      fun getTheValues () = (case !theValues
             of SOME vs => vs
              | NONE => raise Fail "CMBoot: theValues not initialized"
            (* end case *))
      ...
      fun dropPickles () = if #get StdConfig.conserve_memory ()
            then Parse.dropPickles ()
            else ()
      ...
      and run mkSrcPath sflag f s = let
            ...
            val gr = GroupReg.new ()
            in
	      (case Parse.parse (parse_arg (gr, sflag, p))
               of NONE => false
                | SOME (g, gp) => f gp g)
	      before dropPickles ()
            end
      ...
      and load_plugin' p = let
            ...
            val gr = GroupReg.new ()
            val success = ((case Parse.parse (parse_arg (gr, NONE, p))
                   of NONE => false
            ...
            in
              if success
                then Say.vsay ["[plugin ", d, " loaded successfully]\n"]
                else Say.vsay ["[unable to load plugin ", d, "]\n"];
              success
            end

      and load_plugin context s = let
            fun badname s = Say.say ["[bad plugin name: ", s, "]\n"]
            val pp = SrcPath.standard
                  { env = penv, err = badname }
      ...
      fun to_portable s = let
            ...
            fun mkres (g, pl) = { graph = g, imports = pl, nativesrc = nativesrc }
            in
	      Option.map
      ...
      fun sources archos group = let
            val policy = (case archos
                   of NONE => fnpolicy
                    | SOME ao => FilenamePolicy.colocate_generic ao
                  (* end case *))
            fun snam (p, version) = FilenamePolicy.mkStableName policy (p, version)
            fun insert0 (a, f, x) = StringMap.insert (a, f, x)
            fun insert (a, p, x) = insert0 (a, SrcPath.osstring p, x)
            fun add (s, p) = SrcPathSet.add (s, p)

            fun addSources (subgroups, sources, a, v) = let
                  fun findSG p = List.find
                        (fn (p', _, _) => SrcPath.compare (p, p') = EQUAL)
                          subgroups
                  fun one (p, x as { class, derived }, (a, v)) =
                        if SrcPathSet.member (v, p)
                          then (a, v)
                        else if class = "cm"
                          then (case findSG p
                             of NONE => (a, v) (* unused group/library *)
                              | SOME (_, gth, _) => addGroup (p, x, gth, a, add (v, p))
                            (* end case *))
                          else (insert (a, p, x), add (v, p))
                  in
                    SrcPathMap.foldli one (a, v) sources
                  end

            and addGroup (p, x, gth, a, v) = (case gth ()
                   of GG.ERRORGROUP => (a, v)
                    | GG.GROUP{ kind, sources, ... } => (case kind
                         of GG.LIB { kind, version } => (case kind
                               of GG.STABLE _ => (insert0 (a, snam (p, version), x), v)
                                | GG.DEVELOPED d =>
                                    addSources (#subgroups d, sources, insert (a, p, x), v)
                              (* end case *))
                          | GG.NOLIB n =>
                              addSources (#subgroups n, sources, insert (a, p, x), v)
                        (* end case *))
                  (* end case *))
            ...
            fun doit () = (case Parse.parse (parse_arg (gr, NONE, p))
                   of SOME (g, _) => let
                        val (sm, _) =
                              addGroup (p, x0, fn () => g,
                                        StringMap.empty,
                        ...
                        fun add (f, { class, derived }, l) =
                              { file = f, class = class, derived = derived } :: l
                        in
      ...
      fun mk_standalone sflag { setup, project, wrapper, target } = let
            ...
            val gr = GroupReg.new ()
            fun do_cmfile p = (case Parse.parse (parse_arg (gr, NONE, p))
                   of NONE => NONE
                    | SOME (g, gp) => if recomp_runner gp g then SOME (mkBootList g) else NONE
                  (* end case *))
            val setup_list = (case spopt
                   of SOME sp => getOpt (do_cmfile sp, [])
                    | NONE => []
                  (* end case *))
            fun in_setup (i, _) =
                  List.exists (MkBootList.same_info i o #1) setup_list
            fun do_wrapper () = (case do_cmfile wp
                   of NONE => NONE
                    | SOME l => SOME(map #2 (setup_list @ List.filter (not o in_setup) l))
                  (* end case *))
            in
	      (case Parse.parse (parse_arg (gr, sflag, pp))
               of NONE => NONE
                | SOME (g, gp) => if isSome sflag orelse recomp_runner gp g
                    then (case (ts, !(#youngest gp))
                       of (TStamp.TSTAMP tgt_t, TStamp.TSTAMP src_t) =>
			     if Time.< (tgt_t, src_t) then do_wrapper () else SOME []
                        | _ => do_wrapper ()
                      (* end case *))
		    else NONE
              (* end case *))
      ...
      fun redump_heap s : unit = SMLofNJ.Cont.throw (!HostBackend.Interact.redump_heap_cont) s
       ...
       *)
      fun mlbuild buildargs = let
	    fun doit (setup, root, cmfile, heap, listfile, linkargsfile) = let
                  val result = mk_standalone NONE {
                          setup = setup, project = root, wrapper = cmfile, target = heap
                        }
                  in
                    case result
                     of NONE => (
                          Say.say ["Compilation failed.\n"];
                          OS.Process.failure)
                      | SOME [] => (
			  ...
			  in
                            wrf (listfile, concat ["%", Int.toString n, " ", Int.toString m] :: l);
                            wrf (linkargsfile, [concat [" @SMLboot=", listfile]]);
                            OS.Process.success
			  end handle _ => OS.Process.failure
                    (* end case *)
                  end (* doit *)
            val sts = (case buildargs
                   of [root, cmfile, heap, listfile, linkargsfile] =>
                        doit (NONE, root, cmfile, heap, listfile, linkargsfile)
                    | [setup, root, cmfile, heap, listfile, linkargsfile] =>
                        doit (SOME setup, root, cmfile, heap, listfile, linkargsfile)
                    | _ => (
                        Say.say ["bad arguments to @CMbuild\n"];
                        OS.Process.failure)
                  (* end case *))
	    in
              OS.Process.exit sts
      ...
      fun initTheValues (bootdir, de, er, autoload_postprocess, icm) = let
		  ...
		  fun sa (x, d) = SrcPath.set_anchor (penv, x, SOME d)
                  in
                    List.app sa pairList
                  end

            val pidmapfile = P.concat (bootdir, BtNames.pidmap)

            (* read the PIDMAP file; this file consists of a sequence of lines of the form:
             *
             *  <lib> <pos>:<pid> <pos>:<pid> ...
             *
             * where <lib> is a CM path specifying a library (e.g., "$smlnj/internal/intsys.cm")
             * and each <pos>:<pid> pair specifies a file position and persistent ID
             *)
		  ...
		  fun loop m = let
                        fun enter (d, pids) = let
                              fun enter1 (spec, pm) = let
                                    ...
                                    in
                                      case String.tokens (fn c => c = #":") spec
                                       of [pos, hexp] => (case (fromHex hexp, Int.fromString pos)
                                             of (SOME p, SOME i) => (case DE.look de p
                                                   of NONE => pm
                                                    | SOME obj =>
                                                        IM.insert (pm, i, DE.singleton (p, obj))
                                                  (* end case *))
                                             | _ => pm
                                            (* end case *))
                                        | _ => pm
                              ...
                              in
                                SrcPathMap.insert (
                                  m, SrcPath.decode penv d,
                                  List.foldl enter1 IM.empty pids)
                              end (* enter *)
                        in
                          case TextIO.inputLine s
                           of NONE => m
                            | SOME line => (case String.tokens Char.isSpace line
                                 of d :: pids => loop (enter (d, pids))
                                  | _ => loop m
                                (* end case *))
                          (* end case *)
                        end (* loop *)
            ...
            val _ = SafeIO.perform {
                    openIt = fn () => TextIO.openIn pidmapfile,
                    closeIt = TextIO.closeIn,
                    work = readpidmap,
                    cleanup = fn _ => ()
                  }
		...
		| SOME init_group => let
                    ...
                    val { exports = ltm, ... } = Link.newTraversal (
                          init_group,
                          fn _ => raise Fail "init: get bfc?")

                    fun getSymTrav (tr_m, sy) = (case SymbolMap.find (tr_m, sy)
                           of NONE => raise Fail "init: bogus init group (1)"
                            | SOME tr => tr
                          (* end case *))
                    ...
                    fun bare_autoload x = (
                          Say.say ["!* ", x, ": \"autoload\" not available, using \"make\"\n"];
                          makeStd x)
    ...
    fun init (bootdir, de, er, useStream, useFile, errorwrap, icm) = let
	  fun procCmdLine () = let
                      ...
                      fun walk indent (ControlRegistry.RTree rt) = let
                            ...
                            fun one ci = let
                                  ...
                                  in
                                    if padsz < sz
                                      then let
                                        val padsz' = Int.max (lw, sz + 8 + indent)
                                        in
                            ...
                            in
                              case (ctls, subregs)
                               of ([], []) => ()
                                | _ => (
                                    format' Say.say (sp indent o text help o text ":" o nl);
                                    List.app one ctls;
                                    List.app (walk (indent + 1)) subregs)
                              (* end case *)
                            end (* walk *)
                      in
                        walk 2
                ...
                fun help level = (
                      Say.say [optionsMessage];
		      show_controls (
                        Controls.name o #ctl,
                        fn ci => concat ["(", #help (Controls.info (#ctl ci)), ")"],
                        FormatComb.pad FormatComb.left) level)

                fun showcur level = show_controls (
                        fn ci => (Controls.name (#ctl ci) ^ "="),
                        fn ci => Controls.get (#ctl ci),
                        fn _ => fn ff => ff) level

                fun show_envvars level = show_controls (
                        fn ci => (Controls.name (#ctl ci) ^ ":"),
                        fn ci => Option.getOpt (#envName (#info ci), "(none)"),
                        FormatComb.pad FormatComb.left) level

	        fun badopt opt f () = Say.say [
                        "!* bad ", opt, " option: `", f, "'\n",
                ...
                fun carg (opt as ("-C" | "-D"), f, _, _) = let
                      val bad = badopt opt f
                      ...
                      in
                        if name = ""
                               ...
                               of NONE => Say.say ["!* no such control: ", name, "\n"]
                                | SOME sctl => (
                                    Controls.set (sctl, value)
                                      handle Controls.ValueSyntax vse => Say.say [
                          ...
                          then #set (SSV.symval name) (SOME 1)
                          else (case Int.fromString value
                             of SOME i => #set (SSV.symval name) (SOME i)
                              | NONE => bad ()
                            (* end case *))
                      end
                  | carg ("-U", f, _, _) = (case String.extract (f, 2, NONE)
                       of "" => badopt "-U" f ()
                        | var => #set (SSV.symval var) NONE
                      (* end case *))
                  | carg ("-h", f, _, last) = (
                      case String.extract (f, 2, NONE)
                       of "" => help (SOME 0)
                  ...
                  | carg ("-e", f, _, last) = (
                      case String.extract (f, 2, NONE)
                       of "" => show_envvars (SOME 0)
                        | level => show_envvars (Int.fromString level)
                      (* end case *);
                      quit_if last)
                  | carg (_, f, mk, _) = processFile (
                      f, mk, String.map Char.toLower (getOpt (OS.Path.ext f, "<none>")))
                ...
                fun args ([], _) = ()
                  ...
                  | args (f :: rest, mk) = let
                      val arg = if size f < 2 then "" else String.substring(f, 0, 2)
                      in
                        carg (arg, f, mk, List.null rest);
                        nextarg mk
                      end

                and nextarg mk = let
                      val l = SMLofNJ.getArgs ()
                      in
                        SMLofNJ.shiftArgs (); args (l, mk)
                      end
                in
                  case SMLofNJ.getArgs ()
                   of ["@CMslave"] => (#set StdConfig.verbose false; slave ())
	  ...
	  in
            ...
            initTheValues (
              bootdir, de, er,
              fn () => (Cleanup.install initPaths; procCmdLine),
              icm)
	  end (* init *)
