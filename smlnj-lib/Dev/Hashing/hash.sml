 ...
 * All rights reserved.
 *
 * FNV (Fowler-Noll-Vo) hashing combinators.  This API and
 * implementation is loosely based on the OCaml CCHash module
...
(* QUESTION: should the "hash state" be abstract? *)
    ...
    val pair' : ('a t * 'b t) -> ('a * 'b) hash
    val triple : ('a t * 'b t * 'c t) -> ('a * 'b * 'c) t
    val triple' : ('a t * 'b t * 'c t) -> ('a * 'b * 'c) hash
    val quad : ('a t * 'b t * 'c t * 'd t) -> ('a * 'b * 'c * 'd) t
    val quad' : ('a t * 'b t * 'c t * 'd t) -> ('a * 'b * 'c * 'd) hash
...
structure Hash : HASHING =
  struct
    ...
    fun pair' (hf1, hf2) = finish (pair (hf1, hf2))
    fun triple' (hf1, hf2, hf3) = finish (triple (hf1, hf2, hf3))
    fun quad' (hf1, hf2, hf3, hf4) = finish (quad (hf1, hf2, hf3, hf4))
