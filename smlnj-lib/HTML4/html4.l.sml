structure HTML4Lexer
  = struct

    datatype yystart_state = COM2 | COM1 | INTAG | INITIAL
...
fun yyAction11 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
         raise Fail(concat["Unexpected text in comment: \"", yytext, "\""])
      end
...
fun yyAction19 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
         raise Fail(concat["Unexpected character '", yytext, "'"])
      end
       ...
       of NONE => yyAction19(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = 0wx3F
              ...
              then if inp = 0wx40
                  then yyAction19(strm, yyNO_MATCH)
                  else yyQ22(strm', yyMATCH(strm, yyAction19, yyNO_MATCH))
            else if inp = 0wx61
...
in
  ...
  (* end case *))
    ...
    fun lex sm  (STRM (yystrm, memo), ss) = (
          case !memo
           of NONE => let
