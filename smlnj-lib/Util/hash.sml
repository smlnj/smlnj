 ...
 * All rights reserved.
 *
 * Hashing combinators; loosely based on the CCHash module.  This interface is designed
 * to support any computaion of a summary value by iterating over the bytes of a value.
 * Applications include computing hash keys, as well as CRC or SHA signatures.
 *)


structure Hash : HASHING
  where type hstate = Word64.word
  where type hash = Word.word
  = struct
