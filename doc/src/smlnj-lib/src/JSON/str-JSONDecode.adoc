...
`[.kw]#val# int : int decoder`::
  ...
  *JSON* integer value and the exception value `JSONError(Overflow, jv)`
  if the integer is too large to be represented as an `Int.int`.
...
`[.kw]#val# rawObject : (string * JSON.value) list decoder`::
  this decoder returns the fields of a *JSON* object as a labelled list
  of raw *JSON* values.  It will raise the exception value
  `JSONError(NotObject, v)` when the argument `v` is not a *JSON* object.

`[.kw]#val# rawArray : JSON.value vector decoder`::
  this decoder returns the elements of a *JSON* array as a vector
  of raw *JSON* values.  It will raise the exception value
  `JSONError(NotArray, v)` when the argument `v` is not a *JSON* array.
...
`[.kw]#val# try : 'a decoder \-> 'a option decoder`::
  ...
  `d`.  If the decoder `d` fails (_i.e._, raises a `JSONError` exception), then
  `NONE` is returned.  Otherwise, `SOME` is applied to the result od decoding the value.
...
`[.kw]#val# orElse : 'a decoder * 'a decoder \-> 'a decoder`::
  ...
  the decoder d1` and, if that fails (_i.e._, raises the `JSONError` exception),
  tries to decode the argument using `d2`.
...
`[.kw]#val# choose : 'a decoder list \-> 'a decoder`::
  ...
  If all of the decoders fail (_i.e._, they all raise the `JSONError` exception),
  then the exception value `JSONError(Fail "no choice", jv)` is raised, where `jv`
  is the *JSON* value being decoded.  The expression `choose [d1, ..., dn]` is
  equivalent to
+
[source,sml]
------------
