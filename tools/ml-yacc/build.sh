...
#
# tool build script for: ml-yacc
#
# location: $SMLNJ/tools/ml-yacc/build.sh
# options: None

CMD=$0

# cleanup trap: delete BOOTLIST and XXXX files output by (sml @CMbuild), and also
# the bin file .cm/*/export.sml, because this will force recompilation and execution
...
# switch to the ml-yacc/src directory to build the heap image
cd src 
...
$SML -DNO_ML_YACC -DNO_ML_LEX @CMbuild "ml-yacc.cm" "export.cm" "ml-yacc" BOOTLIST XXXX
echo SML: $?
echo `ls -l BOOTLIST`
