...
structure GenSExpPickle : sig
  ...
  (* pairs *)
    ...
    fun sexpPklMod () = ModV.getSExpName BT.asdlTypesId
    fun outStrmTy () = S.CONty([], sexpPklMod() ^ ".outstream")
    fun inStrmTy () = S.CONty([], sexpPklMod() ^ ".instream")

    fun gen anySharing (AST.Module{isPrim=false, id, decls}) = let
          val sign = S.AUGsig(
(* TODO: move Util.sigName to SML view *)
      ...
      | gen _ _ = raise Fail "GenSExpPickle.gen: unexpected primitive module"
    ...
    and genWriter (typeModName, tyId, encoding) = let
          ...
          fun primWriter tyId = concat[
                  ModV.getSExpName BT.asdlTypesId, ".", Util.picklerName tyId
                ]
          (* writer expression for a type *)
          fun tyWriter (E.BASE tyId) = primWriter tyId
            | tyWriter (E.IMPORT(modId, tyId)) = concat[
                  ModV.getSExpName modId, ".", Util.picklerName tyId
                ]
            | tyWriter (E.LOCAL(_, tyId)) = Util.picklerName tyId
         ...
         *)
          ...
          and genTy (arg, E.OPTION ty) =
                S.appExp(
                  funApp ("writeOption", [S.IDexp(tyWriter ty)]),
                  pairExp(outSV, arg))
            | genTy (arg, E.SEQUENCE ty) =
                S.appExp(
                  funApp ("writeSeq", [S.IDexp(tyWriter ty)]),
                  pairExp(outSV, arg))
            | genTy (arg, E.SHARED ty) = raise Fail "shared types not supported yet"
            | genTy (arg, E.TYP ty) = funApp (tyWriter ty, [outSV, arg])
...
(*
          ...
          fun genTag tagTyId = funApp(
                baseReader(SOME BT.asdlTypesId, tagTyId),
                [inSV])
          ...
          and genTy (E.OPTION ty) =
            ...
            | genTy (E.TYP ty) = funApp (baseReader ty, [inSV])
*)
          in
